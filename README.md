...
The project uses two key data files and pre-trained models:
encode_data.csv → Dataset with numerical features after one-hot encoding categorical variables.
...
Recommendation Engine: Uses cosine similarity to find restaurants similar to the user’s preferences.
...
Data Loading
Preloaded CSV files (encoded_data.csv, cleaned_data.csv) provide the basis for preprocessing and recommendation. Encoders and scaler are loaded to transform user input.
...
The cosine similarity computes between the user input vector and all restaurants in encode_data.csv. Based on similarity it returned as recommendations.
...
Recommendation Engine: cosine similarity returns personalized restaurant suggestions.
